<?php
	if($_POST['sync_user']!=NULL || $_POST['sync_user']!=''){
		...
		if($new_entry_array!=NULL || $new_entry_array!=''){
			ehr_connect();
			$array_m_push_update = array();
			...
			foreach($new_entry_array as $entry_array => $new_entry){
				...
				if($new_entry[4]=='m_consult_notes_complaint' || $new_entry[4]=='m_consult_notes_dxclass'){
					...
					//echo "BADBAD". $insert_text;
						insert_query($insert_text, $new_entry[4]);
					}
				/*
				lite_connect($_POST['sync_user']);
					$where_clause = "notes_id = '$new_entry[2]' AND patient_id = '$new_entry[0]'";
					echo "QASDFR SELECT * FROM $new_entry[4] WHERE $where_clause";
					$get_all = mysql_query("SELECT * FROM $new_entry[4] WHERE $where_clause") or die("Error 93 : " .mysql_error());
					echo mysql_num_rows($get_all)."DX SELECT * FROM $new_entry[4] WHERE $where_clause";
		...
		if($update_array!=NULL || $update_array!=''){
			...
			foreach($update_array as $up_array => $update_value){
				...
				if($update_value[4]=='m_consult_ccdev_services' || $update_value[4]=='m_consult_vaccine' || $update_value[4]=='m_consult_ccdev_vaccine'){
				...
				}elseif($update_value[4]=='m_patient_fp_method_service' || $update_value[4]=='m_patient_fp_obgyn' || $update_value[4]=='m_patient_fp_hx' || $update_value[4]=='m_patient_fp_pe' || $update_value[4]=='m_patient_fp_pelvic' || $update_value[4]=='m_consult_mc_prenatal' || $update_value[4]=='m_consult_mc_postpartum' || $update_value[4]=='m_consult_mc_services' || $update_value[4]=='m_consult_mc_vaccine'){
					$insert_text = '';
					lite_connect($_POST['sync_user']);
					...
					switch($update_value[4]):
						...
						case 'm_patient_fp_hx':
							$where_field = 'fp_id';
							break;
						case 'm_patient_fp_pe' || 'm_patient_fp_obgyn' || 'm_patient_fp_pelvic' || 'm_patient_fp_method_service':
							$where_field = 'fp_id';
							break;
						default:
							$where_field = 'mc_id';
					...
					while($get_mc_lite_result = mysql_fetch_array($get_mc_lite_date)){
						...
						switch($update_value[4]):
								...
								case 'm_patient_fp_obgyn':
									...
									break;
								case 'm_patient_fp_method_service':
									$mc_id = $update_value[7];
									$spc_field = 'fp_id';
									$patient_id = $update_value[8];
						...
						}else{ echo $update_value[4]; echo $mc_id;
		...
		echo "<tr style='background-color: #99CCFF;text-align:center; '><td><br />";
			...
			echo "<br /><br />Press Enter to proceed";
	...
	function get_existing_id($table_name, $field_id_name, $lite_id){
		//echo "ASDFFF SELECT emr_specific_id AS id FROM $table_name AS a JOIN m_push_status b ON a.$field_id_name=b.lite_specific_id WHERE table_name='$table_name' AND b.lite_specific_id='$lite_id'";
